...
class args:
    def __init__(self, parse_args):
        ...
        if experiment_lower.startswith('fed_avg') or experiment_lower.startswith('fedavg'):
        ...
        # if self.ex.lower().startswith('fedprox'):
        #     self.min_density = 1.0
        #     self.merge = 'fed_avg'
        #     self.chronous = 'syn'
        #     self.recover = True
        ...
        # self.experiment_name = (+self.sample+('' if self.increase == 2 else '_'+str(self.increase))+('' if self.server_up_speed == 10 else '_'+str(self.server_up_speed)+'_')+'_'+(self.sample_client)+('' if self.sample_data_degree == 0.6 else '_'+str(self.sample_data_degree)) + ('' if self.number_clients == 10 else '_'+str(self.number_clients))+'_'+self.ex+ + ('' if str(self.density) == str(cd) else '_' + str(self.density)) + \
        #                        str('' if self.accumulate == 'wg' else '_' + self.accumulate)+('' if self.patience == 5 else '_'+str(self.patience))+ ('_unclip' if not self.clip else '')+ ('_coeff' if self.use_coeff else '') ) 
        self.experiment_name = (self.sample+('' if self.increase == 2 else '_'+str(self.increase))+('' if self.server_up_speed == 10 else '_'+str(self.server_up_speed)+'_')+'_'+(self.sample_client)+('' if self.sample_data_degree == 0.6 else '_'+str(self.sample_data_degree)) + ('' if self.number_clients == 10 else '_'+str(self.number_clients))+'_'+self.ex+  ('' if str(self.density) == str(cd) else '_' + str(self.density)) + str('' if self.accumulate == 'wg' else '_' + self.accumulate)+('' if self.patience == 5 else '_'+str(self.patience))+ ('_unclip' if not self.clip else '')) 
...
if __name__ == "__main__":
    ...
    model = transformer(bern=True) if fiarse_family else transformer()
