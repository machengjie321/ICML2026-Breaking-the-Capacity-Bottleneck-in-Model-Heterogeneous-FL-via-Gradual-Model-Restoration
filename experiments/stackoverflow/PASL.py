...
class args:
    def __init__(self):
        self.seed = 0
        self.experiment_name = 'PASL_FEMNIST'
...
if __name__ == "__main__":
    ...
    checkpoint = load(os.path.join(
        result_path,'niid_medium__1.0_10_pr_fl_FEMNIST_10_[1.0, 0.5, 0.2, 0.1, 0.05]__25','split',
        'checkpoint.pth'))
    ...
    #     test_model = server.client_test_model
    #     test_loader = server.test_loader
    #     test_model.load_state_dict(sd)
    #     for layer in test_model.prunable_layers:
    #         mask = layer.state_dict()['weight'] != 0
    #         layer.mask.copy_(mask)
    #
    #     loss,acc = test_model.evaluate(test_loader)

    acc_list,loss_list, density, _, _ = server.test_split(0)

    checkpoint = load(os.path.join(
        result_path,'niid_medium__1.0_10_fed_asyn_FEMNIST_10_[1.0, 0.5, 0.2, 0.1, 0.05]__25','split',
        'checkpoint.pth'))
    ...
    acc_list2, loss_list2, density2 = [], [], []
    acc_list2,loss_list2, density2, _, _ = server.test_split(0)
    print(acc_list)
    print(density)
    print(acc_list2)
    print(density2)
