...
from bases.fl.simulation_real.Prune_Recover_FL_fedrolex import FedMapServer, FedMapClient,FedMapFL
from bases.fl.simulation_real.Prune_Recover_FL_fedrolex import  parse_args
from bases.fl.simulation_real.Prune_Recover_FL_fedrolex import unpack_client_state_payload
...
def infer_transformer_model_rate_from_state_dict(state_dict):
    embedding_size = state_dict["transformer_embedding.norm.weight"].shape[0]
    scale = embedding_size / BASE_EMB
    return min(1.0, max(1.0 / BASE_EMB, scale * scale))
    ...
    def init_clients(self, sample):
        if self.method.lower().startswith('heterofl') or  self.method.lower().startswith('fjord') or self.method.lower().startswith('fedrolex'):
            ...
            for i in range(len(self.args.client_density)):
                model = transformer(model_rate = self.args.client_density[i]) 
...
class StackOverflowFedMapClient(FedMapClient):
    ...
    def rebuild_model_from_state_dict(self, state_dict):
        ...
        inferred_model_rate = infer_transformer_model_rate_from_state_dict(state_dict)
        self.model = transformer(model_rate=inferred_model_rate)
        self.model.train()
    ...
    def load_state_dict(self, idx_state_dict):
        _, state_dict, _ = unpack_client_state_payload(idx_state_dict)
...
class args:
    def __init__(self, parse_args):
        ...
        if self.ex.lower().startswith('gmr_heterofl') or self.ex.lower().startswith('gmr_fedrolex'):
        ...
        elif self.ex.lower().startswith('abgmr_heterofl') or self.ex.lower().startswith('abgmr_fedrolex'):
            self.min_density = 0.02
            self.merge = 'heterofl'
            self.chronous = 'asyn'
            self.recover = False
        ...
        if self.sample_client == 'medium':
            ...
            assert False
        # self.client_density =  int(part[1]*self.number_clients)*[0.5] + int(part[2]*self.number_clients)*[0.2]+ int(part[3]*self.number_clients)*[0.1]
...
if __name__ == "__main__":
    ...
    sp = get_FL_sampler(list_indices, MAX_ROUND, NUM_LOCAL_UPDATES * CLIENT_BATCH_SIZE, args.client_selection, NUM_CLIENTS)
