    ...
    def init_test_loader(self):
        ...
        with torch.no_grad():
            ...
            for batch in self.test_loader:
                # 这里的 batch 实际上是 [24, 21] 这样的 2D Tensor (一个用户的多条样本)
                if isinstance(batch, torch.Tensor):
                    # 如果是 2D，我们需要把它拆成 1D Tensor 的列表
                    if batch.dim() == 2:
                        # 使用 unbind 把 batch 维度拆开
                        all_sequences.extend(list(batch.unbind(0)))
                    elif batch.dim() == 1:
                    ...
                    data = batch[0]
                    if data.dim() == 2:
                        all_sequences.extend(list(data.unbind(0)))
                    else:
                        all_sequences.append(data)

            # 现在 all_sequences 里全是 1D Tensor 了
            # pad_sequence 之后会得到 [Total_Samples, Max_Len]
            self.test_data = torch.nn.utils.rnn.pad_sequence(
                all_sequences, batch_first=True, padding_value=0
            ).to(self.device)
            
            self.test_label = None 
            
            # 再次检查，现在应该是 2D 了
            print(f"✅ 修正后 self.test_data shape = {self.test_data.shape}") 
            # 预期 shape 应该是 [1041*24, 21] 左右的规模
