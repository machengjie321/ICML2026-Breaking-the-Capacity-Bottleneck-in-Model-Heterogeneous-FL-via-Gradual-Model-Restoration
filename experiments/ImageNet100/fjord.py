...
from bases.nn.odleaf import resnet18
...
def resolve_imagenet100_init_lr(exp_args):
    ...
    if (
        experiment_lower.startswith("fjord")
...
class ImageNet100FedMapServer(TimedFjordServerMixin, FjordServer):
...
class ImageNet100FedMapClient(FjordClient):
...
if __name__ == "__main__":
    ...
    server = ImageNet100FedMapServer(config, args, model)
    ...
    client_list = [
        ImageNet100FedMapClient(list_models[idx], config, args=args, model_rate=args.client_density[idx])
