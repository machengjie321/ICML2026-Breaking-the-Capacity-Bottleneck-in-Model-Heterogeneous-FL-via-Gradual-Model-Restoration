import os
import torch
from bases.fl.simulation.Prune_increse_FL import FedMapServer, FedMapClient,FedMapFL
...
class INFedMapServer(FedMapServer):
    ...
    def init_test_loader(self):
        self.test_loader = get_data_loader(EXP_NAME, data_type="test",batch_size=100, num_workers=0, shuffle=False, pin_memory=True)
...
class args:
    def __init__(self, parse_args=None):
        ...
        self.experiment_name = self.experiment_name + '_' + str(self.density)+('' if self.chronous == 'syn' else '_'+self.chronous+'_') +str('_recover' if self.recover else '')\
            +'_'+str(self.stal.lower()) +'_'+ str('' if self.stal_a == 0.6 else self.stal_a)+str('' if self.fair == 'un_fair' else '_'+self.fair) + '_' + str(
            ...
            self.weight_decay) + '_' + self.merge + '_' + str('' if self.accumulate=='g' else self.accumulate ) + str('_clr' if self.control_lr else '') + '_' + str('' if
            self.wdn == 10 else self.wdn) + str('_ft' if self.ft else '') + str('_lr_scheduler' if self.lr_scheduler else '') +  str(
