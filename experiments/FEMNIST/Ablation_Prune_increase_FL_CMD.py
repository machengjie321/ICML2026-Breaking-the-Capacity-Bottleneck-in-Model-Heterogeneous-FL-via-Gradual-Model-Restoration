    ...
    def init_optimizer(self):
        ...
        if self.args.lr_scheduler:
            ...
            self.optimizer_wrapper = OptimizerWrapper(self.model, self.optimizer,lr_scheduler.ReduceLROnPlateau(self.optimizer, mode='min', factor=0.8, patience=50,
                                                            verbose=True),clip=self.args.clip)
        else:
...
class args:
    def __init__(self, parse_args):
        ...
        self.device = select_best_gpu(min_memory=6 * 1024)
        ...
        if self.sample_client == 'medium':
            ...
            assert False
        self.client_density = int(self.part[0]*self.number_clients)*[1] + int(self.part[1]*self.number_clients)*[0.5] + int(self.part[2]*self.number_clients)*[0.2]+ int(self.part[3]*self.number_clients)*[0.1]
        ...
        self.experiment_name = ('2'+self.sample+('' if self.increase == 0.2 else '_'+str(self.increase))+('' if self.server_up_speed == 10 else '_'+str(self.server_up_speed)+'_')+'_'+(self.sample_client)+('' if self.sample_data_degree == 0.6 else '_'+str(self.sample_data_degree)) + ('' if self.number_clients == 10 else '_'+str(self.number_clients))+'_'+self.ex+'_'+self.experiment_name +  ('' if str(self.density) == str(cd) else '_' + str(self.density)) + \
                               str('' if self.accumulate == 'wg' else '_' + self.accumulate)+('' if self.patience == 5 else '_'+str(self.patience))+ ('_clip' if self.clip else '') ) 


        self.experiment_name = ('Ablation_'+self.sample+('' if self.interval == 25 else '_'+str(self.interval))+('' if self.increase == 2.0 else '_'+str(self.increase))+('' if self.server_up_speed == 10 else '_'+str(self.server_up_speed)+'_')+'_'+(self.sample_client)+'_'+self.ex+'2')
