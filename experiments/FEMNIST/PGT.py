...
from bases.fl.simulation_real.PGT import FedMapServer, FedMapClient,FedMapFL, parse_args
...
class args:
    def __init__(self, parse_args):
        ...
        self.experiment_name = 'FEMNIST_PGT_no_finetune'
        ...
        self.experiment_name =  (('' if self.server_up_speed == 10 else 'su'+str(self.server_up_speed)+'_')+self.sample+'_'+(self.sample_client+'_')+('' if self.sample_data_degree == 0.6 else '_'+str(self.sample_data_degree)) +('_'+str(self.number_clients))+'_'+self.ex+'_'+self.experiment_name +'_'+str(self.patience)+ '_' + str(self.density) + \
                               '_' + str('' if self.accumulate == 'wg' else '_' + self.accumulate)+ '_' + str(self.interval) )
...
if __name__ == "__main__":
    ...
    client_list = [FEMNISTFedMapClient(list_models[idx], config, use_adaptive, server.list_extra_params[idx], exp_config = server.exp_config, args=args, device = device) for idx in range(args.number_clients)]
