...
from bases.nn.models.leaf import Conv2,Conv2Mnist
...
class MNISTFedMapServer(FedMapServer):
...
class MNISTFedMapClient(FedMapClient):
    def init_optimizer(self):
        self.optimizer = SGD(self.model.parameters(), lr=INIT_LR, momentum=0.5)
...
class args:
    def __init__(self, parse_args):
        self.seed = 0
        self.experiment_name = 'PIF_MNIST'
...
if __name__ == "__main__":
    ...
    server = MNISTFedMapServer(config, args, Conv2Mnist(), seed, SGD, {"lr": config.INIT_LR}, use_adaptive, device)
    ...
    client_list = [MNISTFedMapClient(list_models[idx], config, args.use_adaptive, server.list_extra_params[idx], exp_config = server.exp_config, args=args, device = device) for idx in range(NUM_CLIENTS)]
