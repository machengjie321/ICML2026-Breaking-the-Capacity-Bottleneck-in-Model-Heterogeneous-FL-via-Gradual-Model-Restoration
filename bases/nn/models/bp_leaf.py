...
from .utils import is_conv, is_fc, Scaler
import numpy as np
...
class Conv2(BaseModel):
    def __init__(self, dict_module: dict = None, bern = False, model_rate = 1, hidden_sizes=None):
        if dict_module is None:
            ...
            scaler = Scaler(model_rate**0.5) if model_rate != 1 else nn.Identity()
            scaler = nn.Identity()
            features = nn.Sequential(DenseConv2d(1, hidden_sizes[0], kernel_size=5, padding=2,bern=bern),  # 32x28x28
                                     nn.ReLU(inplace=True),
                                     nn.MaxPool2d(2, stride=2),  # 32x14x14
                                     DenseConv2d(hidden_sizes[0], hidden_sizes[1], kernel_size=5, padding=2,bern=bern),  # 64x14x14
                                     nn.ReLU(inplace=True),
            ...
            classifier = DenseSequential(DenseLinear(hidden_sizes[1] * 7 * 7, hidden_sizes[2], mode="fan_out",bern=bern),
                                         nn.ReLU(inplace=True),
                                        #  DenseLinear(2048, 62, mode="fan_out",bern=bern))
                                         nn.Linear(hidden_sizes[2], 62))

            dict_module["features"] = features
            dict_module["classifier"] = classifier
            self.output_layer_prefix = "classifier.2."
            # --- 获取最后一层 ---
            final_linear_layer = classifier[-1]

            # --- 检查并应用初始化 ---
            if isinstance(final_linear_layer, nn.Linear):
                
                # 1. 权重初始化：应用 Kaiming Uniform，强制使用 'fan_out' 模式
                init.kaiming_uniform_(
                    final_linear_layer.weight, 

                    mode='fan_out', # <<--- 关键：强制 fan_out 模式
    ...
    def to_sparse(self):#Call the to_sparse method of each subnetwork.
...
class Conv2Mnist(BaseModel):
    def __init__(self, dict_module: dict = None):
        if dict_module is None:
            dict_module = dict()
            features = nn.Sequential(DenseConv2d(1, 10, kernel_size=5,),
                                     nn.BatchNorm2d(10),
                                     nn.MaxPool2d(2),
                                     DenseConv2d(10, 20, kernel_size=5),
                                     nn.BatchNorm2d(20),
                                     nn.MaxPool2d(2),)


            classifier = DenseSequential(DenseLinear(320, 50, mode="fan_out"),
                                         nn.ReLU(inplace=True),
                                         DenseLinear(50, 10, mode="fan_out"))
        ...
        super(Conv2Mnist, self).__init__(cross_entropy, dict_module)
    ...
    def forward(self, inputs):
        ...
        # outputs = outputs.view(-1, 320)
        outputs = outputs.view(outputs.size(0), -1)
        outputs = self.classifier(outputs)
        # print(f'output_size : {outputs.size()}')
        # print(outputs)
        return outputs
