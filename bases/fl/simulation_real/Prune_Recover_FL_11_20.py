...
class FedMapServer(ABC):
    ...
    def split_model(self, start):
        ...
        for mask in self.list_mask:
            ...
            list_client_model.append(clean_model)

            sub_model_time.append(timer() - start)

        return list_state_dict, sub_model_time, list_client_model

    @torch.no_grad()
    def test_split(self, start):
        list_state_dict = []
        sub_model_time = []
    ...
    def main(self, idx, list_sd, list_num_proc, lr, list_client_loss, list_client_acc, list_client_sum, round_time,
        ...
        for i in range(len(self.client_density)):
            ...
            if self.need_client_acc and self.train_number[i] % test_client_interval == 1:
                if list_sd[i] != None:
                    ...
                    client_loss, client_acc = self.client_list[i].model.evaluate(subset_data_loader)
                else:
        ...
        if interval_signal:
            ...
            if self.test_split_statu and avg_acc >= self.sp:
                self.save_checkpoint_for_split(self.sp)
            ...
            self.logger.info(
                "fed_avg Loss/acc (at round #{}) = {}/{}   Loss/acc={}/{}".format(self.round, avg_loss, avg_acc, loss,
                                                                                  acc))
            self.logger.info(
            ...
            self.logger.info('sum_server_upload :' + str(self.sum_server_upload[-1]))
        ...
        if interval_signal:
            ...
            start = timer()
            self.list_state_dict, sub_model_time, list_client_model = self.split_model(start)
        ...
        if interval_signal:
            ...
            if (not self.recover) or (self.min_density == 1):
                if self.early_stoping.patience[0] != max(self.patience * 10, 100):
                    self.logger.info('Enter the end stage, increase the patience to obtain the best acc')
                    self.early_stoping.patience[0] = max(self.patience * 10, 100)
            ...
            if self.recover:
                ...
                if old_client_desity != self.client_density:
                    self.logger.info('adjust model' + str(idx) + ': the new density is ' + str(
                        self.client_density))
                    print('adjust model' + str(idx) + ': the new density is ' + str(
                        self.client_density))
        ...
        return self.list_state_dict, self.model_idx, sum_time, self.list_client_loss, self.sub_density, self.list_sparse_state_dict, self.list_sparse_client_dict, list_client_model
    ...
    def main(self):
        ...
        while True:
            ...
            if self.interval_signal:
                ...
                if self.interval_signal and self.server.min_density != 1.0 and idx != 0:
                    if self.server.first_stage or self.args.ex.lower().startswith(
                        ...
                        for i in range(len(self.average_round_time)):
                            ...
                            if new_td >= self.config.min_density or new_td >= self.min_density:
            ...
            list_state_dict, model_idx, sum_time, self.client_loss, sub_density, list_sub_sparse_state_dict, self.list_sparse_client_dict, list_client_model \
            ...
            for i in range(len(self.client_list)):
                if self.client_state[i]:
                    ...
                    if time_from_server_to_client[i] + self.standard_time > self.threshold:
                        list_store_model_client[i] = list_client_model[i]
                    ...
                    if self.server_to_client[i] > 0:
                        if time_from_server_to_client[i] + self.standard_time > self.threshold:
                            ...
                            self.client_download_time[i].append(
                                time_from_server_to_client[i] + self.standard_time - begin_time_client_download[i])
                            self.sum_client_download_time[i] += self.client_download_time[i][-1]
                            list_client_model[i] = list_store_model_client[i]
            ...
            for i in range(len(self.client_state)):
                if self.client_state[i]:
                    self.client_list[i].model = list_client_model[i]
