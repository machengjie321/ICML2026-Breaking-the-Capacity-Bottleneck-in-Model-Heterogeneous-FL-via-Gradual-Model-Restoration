...
class FedMapServer(ABC):
    ...
    def main(self, idx, list_sd, list_num_proc, lr, start, list_loss, list_acc, list_est_time,
        ...
        list_state_dict, model_idx, sub_model_time = self.control.sub_adjust(client_density=client_density,
                                                                             min_density=0.1)
...
class FedMapClient:
    ...
    def main(self):
        ...
        input_list = []
        label_list = []
        locale_update = int((((2-self.calc_model_params())-1)+1)*self.config.NUM_LOCAL_UPDATES)
        ...
        for i in range(locale_update-self.config.NUM_LOCAL_UPDATES):
            inputs, labels = input_list[i], label_list[i]
            # self.optimizer_wrapper.step(inputs,labels)
            num_proc_data += len(inputs)
...
class FedMapFL(ABC):
    ...
    def main(self):
        ...
        for idx in range(self.max_round):
            ...
            list_state_dict, model_idx, sub_time = self.server.main(idx, list_state_dict, list_num, last_lr,
                                                                    server_time,
            ...
            list_model_size = self.get_real_size(list_state_dict, self.server.experiment_name)
            communicate_time_from_server_to_client = simluate_server_to_client(time, list_model_size, server_up,
